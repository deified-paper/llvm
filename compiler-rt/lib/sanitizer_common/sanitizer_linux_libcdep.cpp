...
#if SANITIZER_LINUX && !SANITIZER_ANDROID && (!defined(__MUSL__) || !__MUSL__)
...
# else
  ...
  *size = 0;
# endif
#elif SANITIZER_LINUX && !SANITIZER_ANDROID && defined(__MUSL__) && __MUSL__
  *addr = 0;
  *size = 0;
...
uptr GetTlsSize() {
    ...
    SANITIZER_OPENBSD || SANITIZER_SOLARIS || (defined(__MUSL__) && __MUSL__)
  uptr addr, size;
  GetTls(&addr, &size);
  return size;
